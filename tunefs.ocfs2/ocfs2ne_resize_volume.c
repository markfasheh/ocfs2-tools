...
{
	errcode_t ret;
	uint64_t max_bytes = clusters_to_bytes(fs, UINT32_MAX);
	uint64_t device_bytes;
	uint64_t try_clusters = clusters_in_bytes(fs, new_size);
	uint64_t try_blocks;
	...
	if (try_clusters > UINT32_MAX) {
		verbosef(VL_APP,
			 "Requested %"PRIu64" clusters (encompassing "
			 "%"PRIu64" bytes)\n",
	...
	if (try_blocks > UINT32_MAX) {
		verbosef(VL_APP,
			 "Requested %"PRIu64" clusters (%"PRIu64" "
			 "blocks)\n",
